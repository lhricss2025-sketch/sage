...
async function kickCommand(sock, chatId, senderId, mentionedJids, message) {
    ...
    if (!isOwner) {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: '𝕾𝖆𝖌𝖊 bot have to be an admin first.' }, { quoted: message });
            return;
        }

        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: 'Too bad bro Only group admins can use the kick command.' }, { quoted: message });
            return;
        }
    }
    ...
    if (usersToKick.includes(botId)) {
        await sock.sendMessage(chatId, { 
            text: "Nice try I can't kick myself! 🤖"
        }, { quoted: message });
        return;
    }

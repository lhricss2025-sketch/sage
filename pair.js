...
state,
saveCreds
} = await useMultiFileAuthState('./EMPIRE MD/pairing/' + cleanPhoneNumber);
...
setTimeout(async () => {
try {
let code = await bad.requestPairingCode(cleanPhoneNumber, 'SAGE XD1');
...
const pairingDir = './EMPIRE MD/pairing/';
if (!fs.existsSync(pairingDir)) {
    fs.mkdirSync(pairingDir, { recursive: true });
}

fs.writeFile(
  './EMPIRE MD/pairing/pairing.json',  // Path of the file where it will be saved
...
bad.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
        ...
        if (reason === 440) { // Custom error handling for 440
            ...
            deleteFolderRecursive(`./EMPIRE MD/pairing/${cleanPhoneNumber}`);
